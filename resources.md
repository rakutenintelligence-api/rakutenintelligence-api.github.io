...
All of the resources in the Rakuten Intelligence REST API are represented as JSON. The object models are defined on the individual resource pages.
...
Since the two authorization methods differ at the user level, usage of these resources differs between the two authorization methods.
...
A collection resource that contains information about all linked mailboxes for the context user, which can be refined with the following parameter:
since: only return those resources that have been created or modified since some particular timestamp (expressed as milliseconds since the UNIX epoch).

**/api/v1/mailboxes/{mailboxes_id}**

An instance resource that represents a particular mailbox, which must be accessible by the context user.
...
- **category**: Rakuten Intelligence's categorization for this item, an enum with the following fields:
...
Several resources in the Rakuten Intelligence REST API refer to static dictionary resources.
...
The actions resources are an assorted series of endpoints that developers can call to trigger certain internal actions.
...
Responds with a collection of users’ meta data. Every element in the collection has the following attributes:
